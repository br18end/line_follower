...
int infraPin1 = A0;    // pin del infrarrojos utilizado como entrada digital en el lado izquierdo(#1).
int infraPin2 = A1;    // pin del infrarrojos utilizado como entrada digital en el lado derecho(#2).
int valorInfra1 ;  // Valor inicial de la lectura digital del infrarrojos #1.
int valorInfra2 ;  // Valor inicial de la lectura digital del infrarrojos #2.
int OUTPUT4 = 4;    // Output4 conectada al pin 4. RETROCESO IZQ
int OUTPUT3 = 3;    // Ouput3 conectada al pin 3. AVANCE IZQ
int OUTPUT2 = 6;    // Output3 conectada al pin 6. AVANCE DER
int OUTPUT1 = 7;    // Output4 conectada al pin 7. RETROCESO DER
...
void loop() {
  ...
  if (valorInfra1 == 0) // Si la lectura del infrarrojos#1 es 0, entonces se cumplira una de las siguientes condiciones:
  {
    if (valorInfra2 == 0) // Si la lectura del infrarrojos#2 es 0, es decir los dos sensores estan sobre la linea negra, entonces los dos motores avanzaran en linea recta.
    {
      ...
      analogWrite(OUTPUT2, 200);
      analogWrite(OUTPUT3, 200);
  ...
  { if (valorInfra2 == 0) // Como el sensor#1 esta fuera de la linea negra y el sensor#2 esta sobre la linea negra, entonces solo una rueda gira y esto causara el giro.
    {
      ...
      digitalWrite(OUTPUT1, 50);
      digitalWrite(OUTPUT2, 0);
      digitalWrite(OUTPUT3, 0);
      digitalWrite(OUTPUT4, 50);
    }
  }
}
